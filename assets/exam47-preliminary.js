(() => {
  ...
  function renderFlaggedQuestions(part, label) {
    ...
    return `
      <div class="exam47-official-review">
        ...
        <p>複数の選択肢が正答となる可能性がある問題では、速報回答として該当する番号をすべて表示しています。最終的な取り扱いは公式発表をご確認ください。</p>
  ...
  function renderPart(key, part) {
    ...
    const status = isPreparing ? '準備中' : `回答 ${count}/${answers.length}問${questions.length ? `・問題 ${questions.length}問` : ''}`;
  ...
  function createCard(data) {
    ...
    const statusLabel = isPreparing ? '準備中' : (data?.statusLabel || '回答速報掲載中');
    const updated = data?.updatedAt ? `速報更新：${escapeHtml(data.updatedAt)}` : '速報更新：未掲載';
    const notice = data?.notice || '第47回の回答速報は現在準備中です。';

    card.innerHTML = `
      <div class="exam47-preliminary-head">
        <div>
          <h2 id="exam47PreliminaryTitle">第47回 ME2種 回答速報</h2>
          <p class="small">2026年9月6日実施の第47回 第2種ME技術実力検定試験の回答速報です。</p>
        </div>
      ...
      <div class="exam47-lock">
        <strong>現在は回答速報のみ公開しています</strong>
        演習・正誤判定・解説は、正式な問題・正答の公開後に利用可能となる予定です。
      </div>
      ...
      <p class="exam47-footnote">
        ※ 回答速報は非公式の暫定情報です。正式な正答、複数正答・不適切問題の取り扱い、合否は試験実施団体の公式発表をご確認ください。
      </p>`;
  ...
  function createStandaloneSection(data) {
    ...
    section.innerHTML = `
      <div class="exam47-section-label">第47回</div>
      <div class="exam47-section-title-row">
        <div>
          <h2>最新試験・回答速報</h2>
          <p class="small">第47回の回答速報を掲載しています。</p>
        </div>
      </div>`;
  ...
  function fallbackData() {
    return {
      ...
      statusLabel: '準備中',
      updatedAt: null,
      notice: '第47回の回答速報は現在準備中です。正式な正答・合否については、試験実施団体の公式発表をご確認ください。',
      parts: {
